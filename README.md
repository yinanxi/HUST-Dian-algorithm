...
> 停止条件： 决策树的生长需要设定停止条件，以防止过拟合。常见的停止条件包括： ￼
>•达到预设的最大树深度。  
>•叶节点中的样本数小于设定的阈值。   ￼
>•节点的不纯度（如基尼指数）低于某个阈值。  
...
==
### 4-1 恒等变换技巧
在 MLA 中，引入了一个恒等变换技巧来进一步压缩 KV Cache。具体而言，MLA 使用低秩矩阵对键（Key）和值（Value）进行联合压缩，将高维的 KV 表示映射到一个低维的潜在空间中。在需要计算注意力时，再将这些低维表示映射回高维空间以恢复键和值。这种方法减少了需要存储的 KV 信息量，从而降低了内存占用。  
### 4-2 混合方法  
MLA 还采用了一种混合方法来兼容旋转位置编码（RoPE）。由于 RoPE 与低秩 KV 联合压缩可能存在不兼容的问题，MLA 通过解耦 RoPE 的策略来解决这一问题。具体来说，MLA 使用额外的多头查询（multi-head query）机制，使得位置编码与内容信息分离，从而在保持位置编码优势的同时，实现对 KV 的有效压缩。

## conclude  
可以将传统的多头注意力机制比作每个注意力头都独立地查看完整的键和值信息，这样会导致在处理长序列时，需要存储大量的 KV 信息，占用大量内存。而 MLA 的做法类似于将这些键和值的信息进行“压缩”，只存储其核心内容（低维表示），在需要时再“解压缩”回来。此外，为了确保这种“压缩”和“解压缩”过程不会影响位置信息的处理，MLA 采用了特殊的方法（混合方法）来兼容位置编码技术（如 RoPE），确保模型仍然能够正确地理解序列中的位置信息。MLA 在减少内存占用的同时，保持了模型的性能和对位置信息的敏感度。
